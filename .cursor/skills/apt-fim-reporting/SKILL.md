...
When working on an `experiment-run`, preserve or add all of these sections:
...
Each run should capture, either in frontmatter or in the body:
...
When documenting MCP images, include:
...
When documenting ion column images, include:
...
- Prefer the active APT/FIM pages such as `Experiment Series`, `Experiment Runs`, `Specimens`, `Instrument Configurations`, `Startup Checklists`, `Shutdown Checklists`, and `Data Records`.
...
- Distinguish between series-level planning and run-level execution records.
